...
(define M-state
  (lambda (lis stateList)
    (cond
      [(null? lis) stateList]
      [(eq? (command lis) '=)      (M-state (nextStatement lis) (M-assign (statement lis) stateList))]
      [(eq? (command lis) 'var)    (M-state (nextStatement lis) (M-declare (statement lis) stateList))]
